// Self-driving loop tools: metrics, goals, memory, hypotheses.
// These exist so a coding agent connected to /mcp (see .mcp.json + CLAUDE.md)
// can brief itself on the live app, conclude past experiments against real
// numbers, and decide what to build next. The app provides senses (events),
// a compass (goals), and a notebook (memories/hypotheses); the agent — and
// the tokens thrown at it — provide the intelligence.
...
export const IMPROVE_TOOLS: ToolDefinition[] = [
    ...
    async execute(env) {
      ...
      return {
        ...
        guidance:
          ...
          "a lesson via memory_write); 2) pick the proposed hypothesis with the highest impact " +
          "on the worst goal, or create one from an observation; 3) implement it in code, " +
          "instrument it with track(), ship it; 4) hypothesis_update to 'testing' and " +
          "memory_write the decision.",
      };
    },
  },
  {
    ...
    async execute(env, _request, input) {
      ...
      await env.DB.prepare(
        ...
        .bind(
          ...
          ["active", "achieved", "abandoned"].includes(String(input.status))
            ? String(input.status)
            : "active",
          Date.now(),
        )
  ...
  },
  {
    ...
    async execute(env) {
      ...
      return { goals: await Promise.all(results.map((goal) => goalProgress(env, goal))) };
    },
  },
  {
    name: "memory_write",
    description:
      ...
      "(what a change did), lesson (a durable takeaway).",
    inputSchema: schema(
      {
    ...
    async execute(env, _request, input) {
      ...
      if (!["observation", "decision", "result", "lesson"].includes(kind)) {
  ...
  },
  {
    ...
    async execute(env, _request, input) {
      ...
      if (input.status) {
        if (
          !["proposed", "testing", "confirmed", "refuted", "abandoned"].includes(String(input.status))
        ) {
      ...
      binds.push(id);
      await env.DB.prepare(`UPDATE hypotheses SET ${sets.join(", ")} WHERE id = ?`)
        .bind(...binds)
        .run();
