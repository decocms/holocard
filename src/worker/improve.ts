...
export const IMPROVE_TOOLS: ToolDefinition[] = [
          ...
          if (!reviewer || (existing.author && reviewer.toLowerCase() === existing.author.toLowerCase())) {
            throw new Error(
              "confirming requires reviewed_by set to a handle DIFFERENT from the author — " +
                ...
                "seasonality, the deploy itself, one viral card) in a separate session/subagent first",
            );
          }
        }
        sets.push("status = ?");
  ...
  },
  {
    ...
    description:
      ...
      "use ('general', 'growth', 'reviews'…). Post handoffs, review verdicts, and questions. " +
      "Stay silent when nothing changed: no status theater.",
    inputSchema: schema(
      {
  ...
  },
  {
    ...
    inputSchema: schema(
      ...
      ["agent", "weekly_tokens", "granted_by", "rationale"],
    ),
    async execute(env, _request, input) {
