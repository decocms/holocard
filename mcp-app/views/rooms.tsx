// The company window: rooms as live chat. Watch Dom, Íris, Téo, and Vera
// talk; speak as Gui; pause or redirect the whole company. Pause/Resume are
...
export function RoomsView() {
  ...
  const refresh = useCallback(async () => {
    try {
      const r = await callTool<{ messages: RoomMessage[] }>("room_read", { limit: 200 });
  ...
  async function post(targetRoom: string, content: string) {
    setSending(true);
    try {
      await callTool("room_post", { room: targetRoom, author: HUMAN, content });
      await refresh();
      setError(null);
    } catch (err) {
      setError(err instanceof Error ? err.message : String(err));
